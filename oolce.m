function model = oolce(Q, DevQ, R, Dev, itemdevcold, userdevcold, bothdevcold, k, iterations, maxbadcount, alpha, gamma, lambda, verbose, bias)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 
...
%==============================================================			
       ...
       %Update Gradients
        ...
        %p_vecs(:, size(p_vecs, 1) - 1) = 1;
      	itemcoldRMSE(i) = 0;%coldpredict(U, P, p_vecs, itemdevcold, 'item', m, n, mu);
        usercoldRMSE(i) = 0;%coldpredict(U, P, p_vecs, userdevcold, 'user', m, n, mu);
        bothcoldRMSE(i) = 0;%coldpredict(U, P, p_vecs, bothdevcold, 'both', m, n, mu);
...
%===========================================================================================
        function rmse = coldpredict(U, P, p_vecs, X, type, m, n, mu)
%============================================================================================
...
%==============================================================================================
            ...
            if strcmp(type, 'item') %cold item prediction
                pred = spones(X).*(U*p_vecs);
                rmse = rms(X - pred, nnz(X)); 
            ...
            elseif strcmp(type, 'both')
                ...
                [row, col, val] = find(X);
                rmse = rms( mu*ones(length(col), 1) - val, length(val));
            else
