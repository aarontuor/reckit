...
%==============================================================			
       Q_TW = Q'*W; 
       %Update Gradients
		RminUW_TQ = R - smult(R, U, Q_TW);
		RminUW_TQQ_TW = RminUW_TQ *  Q_TW;       
		U = U + alpha*((1 - gamma)*Rdif*P + RminUW_TQQ_TW - lambda*U);
            if bias U(:, k + 2) = 1; end
        P = P + alpha*((1-gamma)*(R' - smult(R', P, U))*U - lambda*P);
            if bias P(:, k + 1) = 1; end
        W = W + alpha*(gamma*Q*(R' - smult(R', Q'*W, U))*U - lambda*W);
            if bias W(:, k + 1) = 1; end								
        ...
        objHistory(i) = squaredFrob(Rdif) + squaredFrob(RminUW_TQ) + lambda*squaredFrob(U) + lambda*squaredFrob(P) + lambda*squaredFrob(W);		
